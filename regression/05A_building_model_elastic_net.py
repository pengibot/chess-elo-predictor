...
def main():
    print("Started Building Model for Elastic Net...")
    ...
    model_filename = r"elastic_net_model.pkl"
    ...
    elcv_model.fit(X_train_scaled, y_train)

    # Validate the model on the validation set
    y_val_pred = elcv_model.predict(X_val_scaled)
    ...
    y_test_pred = elcv_model.predict(X_test_scaled)
    ...
    with open(models_directory / model_filename, 'wb') as model_file:
        pickle.dump(elcv_model, model_file)
    print(f"Model saved to {(models_directory / model_filename).resolve()}")

    print("Finished Building Model for Elastic Net...")
